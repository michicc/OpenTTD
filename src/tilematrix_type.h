...
/** @file tilematrix_type.h Template type for storing a value per area of the map. */

#ifndef TILEMATRIX_TYPE_H
#define TILEMATRIX_TYPE_H
...
class TileMatrix {
	...
	void AllocateStorage(TileIndex tile)
	{
		...
		/* Add the square the tile is in to the tile area. We do this by adding top-left and bottom-right of the square. */
...
public:
	...
	{
		return TileX(tile) % N == 0 && TileY(tile) % N == 0;
	}
	...
	{
		if (!this->area.Contains(tile)) this->AllocateStorage(tile);
	}
	...
	void Add(const TileArea &area)
	{
		if (!this->area.Contains(area)) {
			this->Add(area.tile);
			this->Add(TileAddXY(area.tile, area.w, area.h));
		}
	}

	/**
	 * Get a reference to the value associated with a tile index.
