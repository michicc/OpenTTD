...
Slope GetFoundationSlope(TileIndex tile, int *z)
{
	...
	do {
		f = CombineFoundations(f, _tile_type_procs[GetTileType(tptr)]->get_foundation_proc(tile, tptr, tileh));
...
void DrawFoundation(TileInfo *ti, Foundation f)
{
	...
	int z = ti->z + ApplyPixelFoundationToSlope(f, &slope);
...
void DrawTile(TileInfo *ti)
{
	...
	Foundation f = FOUNDATION_NONE;
	do {
		TileType tt = ti->tptr != nullptr ? GetTileType(ti->tptr) : MP_VOID;
		f = CombineFoundations(f, _tile_type_procs[tt]->get_foundation_proc(ti->tile, ti->tptr, ti->tileh));
	} while (ti->tptr != nullptr && HasAssociatedTile(ti->tptr++));
	...
	if (ti->tile != INVALID_TILE) DrawBridgeMiddle(ti);
}
...
void DoClearSquare(TileIndex tile)
{
	...
	if (_tile_type_procs[GetTileType(tptr)]->animate_tile_proc != nullptr) DeleteAnimatedTile(tile);
	while (HasAssociatedTile(tptr)) {
		if (_tile_type_procs[GetTileType(tptr + 1)]->animate_tile_proc != nullptr) DeleteAnimatedTile(tile);
