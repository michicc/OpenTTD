...
{
	for (const Tile *rail_tile : RailTileIterator::Iterate(tile)) {
		if (HasSignalOnTrackdir(rail_tile, ReverseTrackdir(td)) && !HasSignalOnTrackdir(rail_tile, td) && IsOnewaySignal(rail_tile, TrackdirToTrack(td))) {
			return true;
		}
	}
	return false;
...
static inline Tile *MakeRailDepot(TileIndex t, Owner o, DepotID did, DiagDirection d, RailType r)
{
	Tile *rail_tile = _m.NewTile(t, MP_RAILWAY);
	SetTileOwner(rail_tile, o);
	rail_tile->m2 = did;
