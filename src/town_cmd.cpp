 ...
 */
static void AnimateTile_Town(TileIndex tile, Tile *tptr)
{
...
static void AddAcceptedCargo_Town(TileIndex tile, Tile *house_tile, CargoArray &acceptance, CargoTypes *always_accepted)
{
	const HouseSpec *hs = HouseSpec::Get(GetHouseType(tile));
...
static bool GrowTownWithBridge(const Town *t, const TileIndex tile, const DiagDirection bridge_dir)
{
	...
	if (slope == SLOPE_FLAT) {
		...
		do {
			...
			bridge_tile += delta;
		} while (IsValidTile(bridge_tile) && ((IsWaterTile(bridge_tile) && !IsSea(bridge_tile)) || IsPlainRailTile(bridge_tile) || (IsNormalRoadTile(bridge_tile) && GetDisallowedRoadDirections(GetTileByType(bridge_tile, MP_ROAD)) != DRD_NONE)));
	} else {
		do {
			if (bridge_length++ >= 11) {
			...
			bridge_tile += delta;
		} while (IsValidTile(bridge_tile) && (IsWaterTile(bridge_tile) || IsPlainRailTile(bridge_tile) || (IsNormalRoadTile(bridge_tile) && GetDisallowedRoadDirections(GetTileByType(bridge_tile, MP_ROAD)) != DRD_NONE)));
	}
...
static CommandCost TerraformTile_Town(TileIndex tile, Tile *tptr, DoCommandFlag flags, int z_new, Slope tileh_new)
{
	if (AutoslopeEnabled()) {
		HouseID house = GetHouseType(tile);
