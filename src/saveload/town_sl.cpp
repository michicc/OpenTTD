	...
	void Load(Town *t) const override
	{
		...
		if (t->cargo_accepted.area.w != 0) {
			...
			if (IsSavegameVersionBefore(SLV_REMOVE_TOWN_CARGO_CACHE)) {
				/* In 11ab3c4e the number of cargo types was changed from 32 to 64. The save/load of Town::cargo_accepted
				 * was not updated, such that only half of the data structure is saved/loaded in savegame versions 199 to 218.
				 * Versions 219+ does not save the data at all. We choose to just drop the old data and regenerate it in AfterLoadGame.*/
