...
bool AfterLoadGame()
{
	...
	if (IsSavegameVersionBefore(SLV_LINKGRAPH_SECONDS)) {
		_settings_game.cargo.linkgraph.recalc_interval *= CalendarTime::SECONDS_PER_DAY;
		_settings_game.cargo.linkgraph.recalc_time     *= CalendarTime::SECONDS_PER_DAY;
	}
	...
	if (IsSavegameVersionBefore(SLV_RESTORE_TOWN_CARGO_CACHE)) {
		...
		for (Industry *ind : Industry::Iterate()) {
			...
			for (auto &p : ind->produced) {
				p.average = p.history[LAST_MONTH].production;
			}
		}
	}
