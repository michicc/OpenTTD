...
static void FixOwnerOfRailTrack(TileIndex t)
{
	...
	if (IsTileType(t, MP_ROAD) && GetRoadTileType(t) == 1 /* ROAD_TILE_CROSSING */) {
		...
		RoadBits bits = HasBit(_m[t].m5, 0) ? ROAD_Y : ROAD_X;
		bool hasroad = HasBit(_m[t].m7, 6);
		bool hastram = HasBit(_m[t].m7, 7);
...
bool AfterLoadGame()
{
		...
		for (TileIndex t = 0; t < map_size; t++) {
			switch (GetTileType(t)) {
				case MP_ROAD:
					...
					SB(_m[t].m7, 6, 2, 1); // Set pre-NRT road type bits for conversion later.
					break;

				case MP_STATION:
					if (IsRoadStop(t)) SB(_m[t].m7, 6, 2, 1);
					break;
	...
	if (IsSavegameVersionBefore(SLV_114)) {
		...
		for (TileIndex t = 0; t < map_size; t++) {
			switch (GetTileType(t)) {
				case MP_ROAD:
					if (fix_roadtypes) SB(_m[t].m7, 6, 2, (RoadTypes)GB(_m[t].m7, 5, 3));
				...
				case MP_STATION:
					...
					if (fix_roadtypes) SB(_m[t].m7, 6, 2, (RoadTypes)GB(_m[t].m3, 0, 3));
	...
	if (IsSavegameVersionBefore(SLV_ROAD_TYPES)) {
		...
		for (TileIndex t = 0; t < map_size; t++) {
			...
			if (has_road) {
				RoadType road_rt = HasBit(_m[t].m7, 6) ? ROADTYPE_ROAD : INVALID_ROADTYPE;
				RoadType tram_rt = HasBit(_m[t].m7, 7) ? ROADTYPE_TRAM : INVALID_ROADTYPE;

				assert(road_rt != INVALID_ROADTYPE || tram_rt != INVALID_ROADTYPE);
				SetRoadTypes(t, road_rt, tram_rt);
				SB(_m[t].m7, 6, 2, 0); // Clear pre-NRT road type bits.
			}
		}
	}
			...
			switch (GetTileType(t)) {
				case MP_RAILWAY:
					if (HasSignals(tile)) {
						...
						SetSignalVariant(tile, TRACK_UPPER, HasBit(tile->m2, 2) ? SIG_SEMAPHORE : SIG_ELECTRIC);
						SetSignalVariant(tile, TRACK_LOWER, HasBit(tile->m2, 6) ? SIG_SEMAPHORE : SIG_ELECTRIC);
						ClrBit(tile->m2, 2);
						ClrBit(tile->m2, 6);
					}
				...
				case MP_ROAD: // Clear PBS reservation on crossing
					if (GetRoadTileType(t) == 1 /* ROAD_TILE_CROSSING */) ClrBit(_m[t].m5, 4);
					break;
			...
			switch (GetTileType(t)) {
				case MP_HOUSE:
					if (GetHouseType(t) >= NEW_HOUSE_OFFSET) {
						...
						SB(_m[t].m3, 5, 1, 0);
						SB(_m[t].m6, 2, 6, std::min(per_proc, 63U));
					}
					break;
	...
	if (IsSavegameVersionBefore(SLV_MULTITILE_DOCKS)) {
		for (TileIndex t = 0; t < map_size; t++) {
			...
			if (IsTileType(t, MP_WATER) || IsTileType(t, MP_RAILWAY) || IsTileType(t, MP_STATION) || IsTileType(t, MP_TUNNELBRIDGE)) {
				SB(_m[t].m1, 7, 1, 0); // Can't use SetDockingTile(t, false) due to assert!
			}
