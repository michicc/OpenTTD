			...
			do {
				if (HasTileByType(tile, MP_RAILWAY)) {
					FOR_ALL_RAIL_TILES(rail_tile, tile) {
						...
						if (!HasSignals(rail_tile)) continue;
...
static void LoadUnloadVehicle(Vehicle *front)
{
	...
	if (front->type == VEH_TRAIN && (!HasTileByType(front->tile, MP_STATION) || GetStationIndex(GetTileByType(front->tile, MP_STATION)) != st->index)) {
