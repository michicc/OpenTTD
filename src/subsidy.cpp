...
bool FindSubsidyPassengerRoute()
{
	...
	if (_settings_game.cargo.GetDistributionType(CT_PASSENGERS) == DT_FIXED) {
		...
		const CargoLink *link = GetRandomCargoLink(src, CT_PASSENGERS, false, false, ST_TOWN);
...
bool FindSubsidyTownCargoRoute()
{
	...
	auto cid = SetCargoBitIterator(town_cargo_produced).begin();
	std::advance(cid, cargo_number);
