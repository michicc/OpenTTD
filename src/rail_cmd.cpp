 ...
 * @return the cost of this operation or an error
 */
static CommandCost RemoveSingleRail(DoCommandFlag flags, TileIndex tile, Tile &rail, Track track)
{
 ...
 */
CommandCost CmdBuildSingleSignal(DoCommandFlag flags, TileIndex tile_index, Track track, SignalType sigtype, SignalVariant sigvar, bool convert_signal, bool skip_existing_signals, bool ctrl_pressed, SignalType cycle_start, SignalType cycle_stop, uint8_t num_dir_cycle, uint8_t signals_copy)
{
 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdConvertRail(DoCommandFlag flags, TileIndex tile_index, TileIndex area_start, RailType totype, bool diagonal)
{
		...
		do {
			...
			if (tt != MP_TUNNELBRIDGE) {
				...
				if (flags & DC_EXEC) { // we can safely convert, too
					...
					FindVehicleOnPos(tile_index, &affected_trains, &UpdateTrainPowerProc);
				}
			}
			...
			switch (tt) {
				...
				default: // MP_STATION
					if (flags & DC_EXEC) YapfNotifyTrackLayoutChange(tile_index, GetRailStationTrack(tile));
...
static std::tuple<CommandCost, bool> RemoveTrainDepot(TileIndex index, Tile &tile, DoCommandFlag flags)
{
	if (_current_company != OWNER_WATER) {
	...
	return {CommandCost(EXPENSES_CONSTRUCTION, _price[PR_CLEAR_DEPOT_TRAIN]), flags & DC_EXEC};
}
...
static void DrawTile_Track(TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	if (IsPlainRail(ti->tile)) {
		...
		if (DrawRoadAsSnowDesert(IsOnSnow(ti->index), roadside)) {
...
static CommandCost TestAutoslopeOnRailTile(TileIndex tile, uint flags, int z_old, Slope tileh_old, int z_new, Slope tileh_new, TrackBits rail_bits)
{
	...
	if (tileh_old != tileh_new) {
		...
		if ((flags & DC_EXEC) != 0) MakeClearGrass(tile);
	}
	return  cost;
}
...
static CommandCost TerraformTile_Track(TileIndex index, Tile tile, DoCommandFlag flags, int z_new, Slope tileh_new)
{
	...
	if (IsPlainRail(tile)) {
		...
		if (was_water && HasVehicleOnPos(index, nullptr, &EnsureNoShipProc)) return CommandCost(STR_ERROR_SHIP_IN_THE_WAY);
