 ...
 */
static CommandCost CheckRailTiles(TileIndex tile, Track track, RailType railtype, DoCommandFlag flags)
{
	bool need_convert = false;

	/* Check if the track is compatible with all rail tiles. */
	FOR_ALL_RAIL_TILES(rail_tile, tile) {
		if (!IsPlainRail(rail_tile)) return DoCommand(tile, 0, 0, flags, CMD_LANDSCAPE_CLEAR); // just get appropriate error message
	...
	if (need_convert) return DoCommand(tile, tile, railtype, flags, CMD_CONVERT_RAIL);
...
CommandCost CmdBuildSingleRail(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (HasTileByType(tile, MP_RAILWAY)) {
		...
		ret = CheckRailTiles(tile, track, railtype, flags);
		if (ret.Failed()) return ret;
		cost.AddCost(ret);
		...
		if (flags & DC_EXEC) {
			...
			FOR_ALL_RAIL_TILES(cur_tile, tile) {
		...
		if (HasTileByType(tile, MP_ROAD)) {
			...
			FOR_ALL_ROAD_TILES(road_tile, tile) {
				...
				if (cur_rt == ROADTYPES_TRAM && Company::IsValidID(GetTileOwner(road_tile)) && HasExactlyOneBit(GetRoadBits(road_tile))) {
					CommandCost ret = CheckOwnership(GetTileOwner(road_tile));
					if (ret.Failed()) return ret;
				}
			...
			if ((track == TRACK_X && (rbs & ROAD_X) == ROAD_NONE) || (track == TRACK_Y && (rbs & ROAD_Y) == ROAD_NONE)) {
				Tile *road_tile = GetRoadTileByType(tile, ROADTYPE_ROAD);
				Tile *tram_tile = GetRoadTileByType(tile, ROADTYPE_TRAM);

				/* Crossings must always have a road... */
				uint num_new_road_pieces = 2 - (road_tile != NULL ? CountBits(GetRoadBits(road_tile)) : 0);
				uint num_new_tram_pieces = tram_tile != NULL ? 2 - CountBits(GetRoadBits(tram_tile)) : 0;
				...
				if (flags & DC_EXEC) {
					...
					if (num_new_tram_pieces > 0) {
						SetRoadBits(tram_tile, track == TRACK_X ? ROAD_Y : ROAD_X);
						Owner tram_owner = GetTileOwner(tram_tile);
						if (Company::IsValidID(tram_owner)) {
							Company::Get(tram_owner)->infrastructure.road[ROADTYPE_ROAD] += num_new_tram_pieces;
					...
					if (!HasBit(rts, ROADTYPE_ROAD)) {
						...
						Company::Get(_current_company)->infrastructure.road[ROADTYPE_ROAD] += num_new_road_pieces;
					} else if (num_new_road_pieces > 0) {
						SetRoadBits(road_tile, track == TRACK_X ? ROAD_Y : ROAD_X);
...
CommandCost CmdBuildSingleSignal(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (HasBit(p1, 17) && HasSignalOnTrack(rail_tile, track)) return CommandCost();
	...
	if (!HasSignalOnTrack(rail_tile, track)) {
		...
		if (p2 != 0 && sigvar != GetSignalVariant(rail_tile, track)) {
	...
	if (flags & DC_EXEC) {
		...
		if (p2 == 0) {
			...
			SetPresentSignals(rail_tile, (GetPresentSignals(rail_tile) & ~SignalOnTrack(track)) | (p2 & SignalOnTrack(track)));
			SetSignalVariant(rail_tile, track, sigvar);
			SetSignalType(rail_tile, track, sigtype);
		}
...
static CommandCost CmdSignalTrackHelper(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	if (HasSignalOnTrack(rail_tile, track)) {
		...
		semaphores = GetSignalVariant(rail_tile, track) != SIG_ELECTRIC;
...
static void MergeRailTiles(TileIndex tile)
{
	Tile *prev_tile = GetTileByType(tile, MP_RAILWAY);
	if (prev_tile == NULL) return;

	for (Tile *cur_tile = GetNextTileByType(prev_tile, MP_RAILWAY); cur_tile != NULL; cur_tile = GetNextTileByType(cur_tile, MP_RAILWAY)) {
		...
		if (cur_rt != prev_rt) {
			...
			if (_settings_game.vehicle.disable_elrails &&
					((prev_rt == RAILTYPE_RAIL && cur_rt == RAILTYPE_ELECTRIC) || (prev_rt == RAILTYPE_ELECTRIC && cur_rt == RAILTYPE_RAIL))) {
...
CommandCost CmdConvertRail(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	for (; (tile = *iter) != INVALID_TILE; ++(*iter)) {
		...
		} while ((tptr = GetNextTileByType(tptr, MP_RAILWAY)) != NULL);
...
static inline void DrawTrackSprite(SpriteID sprite, PaletteID pal, const TileInfo *ti, Slope s, const SubSprite *sub = NULL)
{
...
static void DrawTrackBitsOverlay(TileInfo *ti, TrackBits track, const RailtypeInfo *rti, bool draw_halftile, Corner halftile_corner)
{
	...
	if (!draw_halftile) {
		if (ti->tileh == SLOPE_NW && track == TRACK_BIT_Y) {
...
static void GetTileDesc_Track(TileIndex tile, Tile *tptr, TileDesc *td)
{
	...
	switch (GetRailTileType(tptr)) {
		...
		case RAIL_TILE_SIGNALS: {
			...
			if (HasSignalOnTrack(tptr, TRACK_UPPER)) {
				primary_signal = GetSignalType(tptr, TRACK_UPPER);
				secondary_signal = HasSignalOnTrack(tptr, TRACK_LOWER) ? GetSignalType(tptr, TRACK_LOWER) : primary_signal;
			} else {
				secondary_signal = primary_signal = GetSignalType(tptr, TRACK_LOWER);
			}
...
static bool ChangeTileOwner_Track(TileIndex tile, Tile *tptr, Owner old_owner, Owner new_owner)
{
	...
	if (new_owner != INVALID_OWNER) {
		...
		if (HasSignals(tptr)) {
			uint num_sigs = CountBits(GetPresentSignals(tptr));
		...
		bool deleted;
		ClearTile_Track(tile, tptr, DC_EXEC | DC_BANKRUPT, &deleted);
		return !deleted;
	}
	return true;
}
...
static CommandCost TerraformTile_Track(TileIndex tile, Tile *tptr, DoCommandFlag flags, int z_new, Slope tileh_new)
{
	int z_old;
	Slope tileh_old = GetTileSlope(tile, &z_old);
	if (IsPlainRail(tptr)) {
		TrackBits rail_bits = GetTrackBits(tptr);
		...
		CommandCost autoslope_result = TestAutoslopeOnRailTile(tile, tptr, flags, z_old, tileh_old, z_new, tileh_new, rail_bits);
