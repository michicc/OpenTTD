...
static CommandCost CheckRailTiles(TileIndex tile, Track track, RailType railtype, DoCommandFlag flags)
{
	...
	FOR_ALL_RAIL_TILES(rail_tile, tile) {
		if (!IsPlainRail(rail_tile)) return_cmd_error(STR_ERROR_IMPOSSIBLE_TRACK_COMBINATION);
...
static void DrawTrackDetails(const TileInfo *ti, const RailtypeInfo *rti, bool draw_halftile)
{
	...
	SpriteID base_image = GetCustomRailSprite(rti, ti->tile, RTSG_FENCES, draw_halftile ? TCX_UPPER_HALFTILE : TCX_NORMAL);
	if (base_image == 0) base_image = SPR_TRACK_FENCE_FLAT_X;

	switch (GetRailFenceType(ti->tptr)) {
		case RAIL_FENCE_NW:     DrawTrackFence_NW(ti, base_image);    break;
		case RAIL_FENCE_SE:     DrawTrackFence_SE(ti, base_image);    break;
		case RAIL_FENCE_SENW:   DrawTrackFence_NW_SE(ti, base_image); break;
		case RAIL_FENCE_NE:     DrawTrackFence_NE(ti, base_image);    break;
		case RAIL_FENCE_SW:     DrawTrackFence_SW(ti, base_image);    break;
		case RAIL_FENCE_NESW:   DrawTrackFence_NE_SW(ti, base_image); break;
		case RAIL_FENCE_VERT1:  DrawTrackFence_NS_1(ti, base_image);  break;
		case RAIL_FENCE_VERT2:  DrawTrackFence_NS_2(ti, base_image);  break;
		case RAIL_FENCE_HORIZ1: DrawTrackFence_WE_1(ti, base_image);  break;
		case RAIL_FENCE_HORIZ2: DrawTrackFence_WE_2(ti, base_image);  break;
		default: break;
	}
}
	...
	if (IsPlainRail(ti->tptr)) {
		...
		if (rti->UsesOverlay()) {
			...
			switch (GetRailDepotDirection(ti->tptr)) {
				...
				case DIAGDIR_SW: image = ground + RTO_X; break;
				case DIAGDIR_NW: if (!IsInvisibilitySet(TO_BUILDINGS)) break; // else FALL THROUGH
				case DIAGDIR_SE: image = ground + RTO_Y; break;
				default: break;
			}
		...
		if (HasCatenaryDrawn(GetRailType(ti->tptr))) DrawCatenary(ti);
