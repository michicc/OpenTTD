 ...
 */
static CommandCost CheckRailTiles(TileIndex tile, Track track, RailType railtype, bool auto_remove_signals, DoCommandFlag flags)
{
	CommandCost cost;
	...
	for (const Tile *rail_tile : RailTileIterator::Iterate(tile)) {
		if (!IsPlainRail(rail_tile)) return Command<CMD_LANDSCAPE_CLEAR>::Do(flags, tile); // just get appropriate error message

		if (TrackOverlapsTracks(GetTrackBits(rail_tile), track)) {
			/* Possible target rail tile. */
			CommandCost ret = CheckTileOwnership(tile, rail_tile);
			if (ret.Failed()) return ret;

			RailType cur_rt = GetRailType(rail_tile);
			if (!IsCompatibleRail(cur_rt, railtype)) return_cmd_error(STR_ERROR_IMPOSSIBLE_TRACK_COMBINATION);

			if (HasSignals(rail_tile) && TracksOverlap(GetTrackBits(rail_tile) | TrackToTrackBits(track))) {
				...
				for (Track track_it = TRACK_BEGIN; track_it < TRACK_END; track_it++) {
					if (HasTrack(rail_tile, track_it) && HasSignalOnTrack(rail_tile, track_it)) {
			...
			if (cur_rt != railtype) {
				if (!HasPowerOnRail(railtype, cur_rt)) {
					/* Engines of the new type are not powered on the present type. Convert
					 * rail type if engines of the present rail type are powered on the new
					 * type, otherwise error out. */
			...
			if (HasSignals(rail_tile)) return_cmd_error(STR_ERROR_MUST_REMOVE_SIGNALS_FIRST);
		}
	}
...
CommandCost CmdBuildSingleRail(DoCommandFlag flags, TileIndex tile, RailType railtype, Track track, bool auto_remove_signals)
{
	...
	if (HasTileByType(tile, MP_RAILWAY)) {
		...
		if (flags & DC_EXEC) {
			Tile *rail_tile = nullptr;

			/* Check if the track overlaps an already present rail tile. */
			for (Tile *cur_tile : RailTileIterator::Iterate(tile)) {
				...
				if (HasBit(tracks, TrackToOppositeTrack(track))) {
					if (GetTileOwner(cur_tile) != _current_company) continue;
					if (GetRailType(cur_tile) != railtype) continue;
					rail_tile = cur_tile;
					break;
				}
			}
			...
			if (rail_tile == nullptr) {
				...
				SetTrackBits(rail_tile, bits | trackbit);
				SetRailFenceType(rail_tile, RAIL_FENCE_NONE);
				MakeClearGrass(tile); // Make barren ground.

				/* Subtract old infrastructure count. */
				uint pieces = CountBits(bits);
				if (TracksOverlap(bits)) pieces *= pieces;
				Company::Get(GetTileOwner(rail_tile))->infrastructure.rail[GetRailType(rail_tile)] -= pieces;
				/* Add new infrastructure count. */
				pieces = CountBits(bits | trackbit);
				if (TracksOverlap(bits | trackbit)) pieces *= pieces;
				Company::Get(GetTileOwner(rail_tile))->infrastructure.rail[GetRailType(rail_tile)] += pieces;
				DirtyCompanyInfrastructureWindows(GetTileOwner(rail_tile));
			}
		}
	} else {
		...
		if (IsTileType(tile, MP_WATER) && IsSlopeWithOneCornerRaised(tileh)) {
			assert(!HasTileByType(tile, MP_TREES));
...
CommandCost CmdRemoveSingleRail(DoCommandFlag flags, TileIndex tile, Track track)
{
	...
	if (rail_tile != nullptr) {
		...
		cost.AddCost(RailClearCost(GetRailType(rail_tile)));
		...
		if (flags & DC_EXEC) {
			...
			if (present == 0) {
				...
				if (!IsTileType(tile, MP_ROAD) && (!IsTileType(tile, MP_WATER) || !IsSlopeWithOneCornerRaised(GetTileSlope(tile, nullptr)))) {
					MakeClearGrass(tile);
				}
				if (!HasTileByType(tile, MP_RAILWAY)) DeleteNewGRFInspectWindow(GSF_RAILTYPES, tile);
			} else {
				SetTrackBits(rail_tile, present);
				SetTrackReservation(rail_tile, GetRailReservationTrackBits(rail_tile) & present);
			}
		}
	} else {
...
CommandCost CmdBuildSingleSignal(DoCommandFlag flags, TileIndex tile, Track track, SignalType sigtype, SignalVariant sigvar, bool convert_signal, bool skip_existing_signals, bool ctrl_pressed, SignalType cycle_start, SignalType cycle_stop, uint8 num_dir_cycle, byte signals_copy)
{
	...
	/* You can only build signals on plain rail tiles. */
	Tile *rail_tile = GetRailTileFromTrack(tile, track);
	...
	if (TracksOverlap(GetTrackBits(rail_tile))) return_cmd_error(STR_ERROR_NO_SUITABLE_RAILROAD_TRACK);

	/* In case we don't want to change an existing signal, return without error. */
	if (skip_existing_signals && HasSignalOnTrack(rail_tile, track)) return CommandCost();

	/* you can not convert a signal if no signal is on track */
	if (convert_signal && !HasSignalOnTrack(rail_tile, track)) return_cmd_error(STR_ERROR_THERE_ARE_NO_SIGNALS);
	...
	if (!HasSignalOnTrack(rail_tile, track)) {
		...
		if (signals_copy != 0 && sigvar != GetSignalVariant(rail_tile, track)) {
			/* convert signals <-> semaphores */
			cost = CommandCost(EXPENSES_CONSTRUCTION, _price[PR_BUILD_SIGNALS] + _price[PR_CLEAR_SIGNALS]);

		} else if (convert_signal) {
			/* convert button pressed */
			if (ctrl_pressed || GetSignalVariant(rail_tile, track) != sigvar) {
	...
	if (flags & DC_EXEC) {
		...
		if (!HasSignals(rail_tile)) {
			...
			SetHasSignals(rail_tile, true);
			SetSignalStates(rail_tile, 0xF); // all signals are on
			SetPresentSignals(rail_tile, 0); // no signals built by default
			SetSignalType(rail_tile, track, sigtype);
			SetSignalVariant(rail_tile, track, sigvar);
		}

		/* Subtract old signal infrastructure count. */
		Company::Get(GetTileOwner(rail_tile))->infrastructure.signal -= CountBits(GetPresentSignals(rail_tile));

		if (signals_copy == 0) {
			if (!HasSignalOnTrack(rail_tile, track)) {
				/* build new signals */
				SetPresentSignals(rail_tile, GetPresentSignals(rail_tile) | (IsPbsSignal(sigtype) ? KillFirstBit(SignalOnTrack(track)) : SignalOnTrack(track)));
				SetSignalType(rail_tile, track, sigtype);
				SetSignalVariant(rail_tile, track, sigvar);
				while (num_dir_cycle-- > 0) CycleSignalSide(rail_tile, track);
			} else {
				if (convert_signal) {
					/* convert signal button pressed */
					if (ctrl_pressed) {
						/* toggle the present signal variant: SIG_ELECTRIC <-> SIG_SEMAPHORE */
						SetSignalVariant(rail_tile, track, (GetSignalVariant(rail_tile, track) == SIG_ELECTRIC) ? SIG_SEMAPHORE : SIG_ELECTRIC);
						...
						SetSignalType(rail_tile, track, sigtype);
						SetSignalVariant(rail_tile, track, sigvar);
						if (IsPbsSignal(sigtype) && (GetPresentSignals(rail_tile) & SignalOnTrack(track)) == SignalOnTrack(track)) {
							SetPresentSignals(rail_tile, (GetPresentSignals(rail_tile) & ~SignalOnTrack(track)) | KillFirstBit(SignalOnTrack(track)));
						}
					}

				} else if (ctrl_pressed) {
					/* cycle between cycle_start and cycle_end */
					sigtype = (SignalType)(GetSignalType(rail_tile, track) + 1);
					...
					CycleSignalSide(rail_tile, track);
					/* Query current signal type so the check for PBS signals below works. */
					sigtype = GetSignalType(rail_tile, track);
				}
			}
			...
			SetPresentSignals(rail_tile, (GetPresentSignals(rail_tile) & ~SignalOnTrack(track)) | (signals_copy & SignalOnTrack(track)));
			SetSignalVariant(rail_tile, track, sigvar);
			SetSignalType(rail_tile, track, sigtype);
		}
		...
		if (IsPbsSignal(sigtype)) {
			...
			uint mask = GetPresentSignals(rail_tile) & SignalOnTrack(track);
			SetSignalStates(rail_tile, (GetSignalStates(rail_tile) & ~mask) | ((HasBit(GetRailReservationTrackBits(rail_tile), track) && EnsureNoVehicleOnGround(tile).Succeeded() ? UINT_MAX : 0) & mask));
		}
		MarkTileDirtyByTile(tile);
		AddTrackToSignalBuffer(tile, track, _current_company);
		YapfNotifyTrackLayoutChange(tile, track);
...
CommandCost CmdRemoveSingleSignal(DoCommandFlag flags, TileIndex tile, Track track)
{
	...
	if (flags & DC_EXEC) {
		...
		if (HasReservedTracks(tile, TrackToTrackBits(track))) {
			v = GetTrainForReservation(tile, track);
		} else if (IsPbsSignal(GetSignalType(rail_tile, track))) {
			...
			for (int i = 0; v == nullptr && i < 2; i++, td = ReverseTrackdir(td)) {
				...
				if (!HasSignalOnTrackdir(rail_tile, ReverseTrackdir(td))) continue;
		...
		Company::Get(GetTileOwner(rail_tile))->infrastructure.signal -= CountBits(GetPresentSignals(rail_tile));
		SetPresentSignals(rail_tile, GetPresentSignals(rail_tile) & ~SignalOnTrack(track));
		Company::Get(GetTileOwner(rail_tile))->infrastructure.signal += CountBits(GetPresentSignals(rail_tile));
		DirtyCompanyInfrastructureWindows(GetTileOwner(rail_tile));

		/* removed last signal from tile? */
		if (GetPresentSignals(rail_tile) == 0) {
			SetSignalStates(rail_tile, 0);
			SetHasSignals(rail_tile, false);
			SetSignalVariant(rail_tile, INVALID_TRACK, SIG_ELECTRIC); // remove any possible semaphores
		}

		AddTrackToSignalBuffer(tile, track, GetTileOwner(rail_tile));
		YapfNotifyTrackLayoutChange(tile, track);
...
/**
 * Merge the associated rail tiles if possible.
 * @param tile Tile index to check.
 * @param start_tile First associated tile that may be merged or \c nullptr if all associated tiles should be checked.
 * @return True if the start_tile was deleted.
 */
static bool MergeRailTiles(TileIndex tile, Tile *start_tile = nullptr)
{
	Tile *prev_tile = GetTileByType(tile, MP_RAILWAY);

	/* If a start tile is given, find the tile previous to it. */
	if (start_tile != nullptr && start_tile != prev_tile) {
		while (GetNextTileByType(prev_tile, MP_RAILWAY) != start_tile) prev_tile = GetNextTileByType(prev_tile, MP_RAILWAY);
	}

	if (prev_tile == nullptr) return false;

	bool deleted_start = false;
	for (Tile *cur_tile = GetNextTileByType(prev_tile, MP_RAILWAY); cur_tile != nullptr; cur_tile = GetNextTileByType(cur_tile, MP_RAILWAY)) {
		/* Can only merge plain rail tiles. */
		if (!IsPlainRail(cur_tile) || !IsPlainRail(prev_tile)) continue;
		/* Can only merge same owner. */
		if (GetTileOwner(cur_tile) != GetTileOwner(prev_tile)) continue;

		RailType prev_rt = GetRailType(prev_tile);
		RailType cur_rt = GetRailType(cur_tile);
		/* Different rail type. */
		if (cur_rt != prev_rt) {
			/* 'Hidden' elrail means we can merge rail and elrail. */
			if (_settings_game.vehicle.disable_elrails && ((prev_rt == RAILTYPE_RAIL && cur_rt == RAILTYPE_ELECTRIC) || (prev_rt == RAILTYPE_ELECTRIC && cur_rt == RAILTYPE_RAIL))) {
				/* Force rail type to elrail. */
				SetRailType(prev_tile, RAILTYPE_ELECTRIC);
			} else {
				continue;
			}
		}
		...
		TrackBits prev_bits = GetTrackBits(prev_tile);
		TrackBits cur_bits = GetTrackBits(cur_tile);

		/* Subtract old infrastructure counts. */
		uint prev_count = CountBits(prev_bits);
		uint cur_count = CountBits(cur_bits);
		if (TracksOverlap(prev_bits)) prev_count *= prev_count;
		if (TracksOverlap(cur_bits))  cur_count *= cur_count;
		c->infrastructure.rail[prev_rt] -= prev_count;
		c->infrastructure.rail[cur_rt]  -= cur_count;

		/* Add merged infrastructure count. */
		prev_count = CountBits(prev_bits | cur_bits);
		if (TracksOverlap(prev_bits | cur_bits)) prev_count *= prev_count;
		c->infrastructure.rail[GetRailType(prev_tile)] += prev_count; // We may have changed the rail type to elrail.

		DirtyCompanyInfrastructureWindows(c->index);

		/* Transplant track bits and signal states. */
		SetTrackBits(prev_tile, prev_bits | cur_bits);
		SetTrackReservation(prev_tile, GetRailReservationTrackBits(prev_tile) | GetRailReservationTrackBits(cur_tile));
		if (HasSignals(cur_tile)) {
			if (!HasSignals(prev_tile)) SetHasSignals(prev_tile, true);
			SetPresentSignals(prev_tile, GetPresentSignals(prev_tile) | GetPresentSignals(cur_tile));
			SetSignalStates(prev_tile, GetSignalStates(prev_tile) | GetSignalStates(cur_tile));
			SetSignalVariant(prev_tile, TRACK_LOWER, (SignalVariant)(GetSignalVariant(prev_tile, TRACK_LOWER) | GetSignalVariant(cur_tile, TRACK_LOWER)));
			SetSignalVariant(prev_tile, TRACK_UPPER, (SignalVariant)(GetSignalVariant(prev_tile, TRACK_UPPER) | GetSignalVariant(cur_tile, TRACK_UPPER)));
		}

		/* Remove tile and fix up tile pointer. */
		if (cur_tile == start_tile) deleted_start = true;
		_m.RemoveTile(tile, cur_tile);
		cur_tile = prev_tile;
	}
...
CommandCost CmdConvertRail(DoCommandFlag flags, TileIndex tile, TileIndex area_start, RailType totype, bool diagonal)
{
	...
	for (; (tile = *iter) != INVALID_TILE; ++(*iter)) {
		Tile *tptr = GetTileByType(tile, MP_RAILWAY);
		TileType tt = MP_RAILWAY;
		...
		do {
			...
			if (tt != MP_TUNNELBRIDGE) {
				...
				if (flags & DC_EXEC) { // we can safely convert, too
					TrackBits reserved = (tt == MP_RAILWAY) ? GetReservedRailTracks(tptr) : GetReservedTrackbits(tile);
					Track     track;
			...
			switch (tt) {
				case MP_RAILWAY:
					switch (GetRailTileType(tptr)) {
						...
						default: // RAIL_TILE_NORMAL, RAIL_TILE_SIGNALS, RAIL_TILE_CROSSING
							...
							found_convertible_track = true;
							cost.AddCost(RailConvertCost(type, totype) * CountBits(GetTrackBits(tptr)));
							break;
					}
					break;
				...
				case MP_TUNNELBRIDGE:
				{
					...
					if (flags & DC_EXEC) {
						...
						SetRailType(_m.ToTile(tile), totype);
						SetRailType(_m.ToTile(endtile), totype);
		...
		} while ((tptr = GetNextTileByType(tptr, MP_RAILWAY)) != nullptr);

		if (flags & DC_EXEC) MergeRailTiles(tile);
	}
...
static CommandCost RemoveTrainDepot(TileIndex tile, Tile *rail_tile, DoCommandFlag flags, bool &tile_deleted)
{
	...
	if (flags & DC_EXEC) {
		...
		Company::Get(owner)->infrastructure.rail[GetRailType(rail_tile)]--;
		DirtyCompanyInfrastructureWindows(owner);
...
static CommandCost ClearTile_Track(TileIndex tile, Tile *rail_tile, DoCommandFlag flags, bool &tile_deleted)
{
	CommandCost cost(EXPENSES_CONSTRUCTION);

	if (flags & DC_AUTO) {
		if (!IsTileOwner(rail_tile, _current_company)) {
	...
	switch (GetRailTileType(rail_tile)) {
		...
		case RAIL_TILE_NORMAL: {
			...
			if (flags & DC_EXEC) tile_deleted = true;

			return cost;
		}

		case RAIL_TILE_DEPOT:
			return RemoveTrainDepot(tile, rail_tile, flags, tile_deleted);
...
static void DrawSingleSignal(TileIndex tile, const Tile *rail_tile, const RailtypeInfo *rti, Track track, SignalState condition, SignalOffsets image, uint pos)
{
	bool side;
	...
	SignalType type       = GetSignalType(rail_tile, track);
	SignalVariant variant = GetSignalVariant(rail_tile, track);
...
static void DrawTrackDetails(const TileInfo *ti, const RailtypeInfo *rti, bool draw_halftile)
{
	...
	uint num_sprites = 0;
	SpriteID base_image = GetCustomRailSprite(rti, ti->tile, RTSG_FENCES, draw_halftile ? TCX_UPPER_HALFTILE : TCX_NORMAL, &num_sprites);
	if (base_image == 0) {
	...
	switch (GetRailFenceType(ti->tptr)) {
...
static void DrawTrackBitsOverlay(TileInfo *ti, TrackBits track, const RailtypeInfo *rti, bool draw_halftile, Corner halftile_corner)
{
	...
	TrackBits pbs = _settings_client.gui.show_track_reservation ? GetRailReservationTrackBits(ti->tptr) : TRACK_BIT_NONE;
...
static void DrawTrackBits(TileInfo *ti, TrackBits track, bool draw_halftile, Corner halftile_corner)
{
	...
	if (!draw_halftile) {
		...
		if (IsTileType(ti->tile, MP_CLEAR)) {
			/* Adjust drawn sprite for better appearance. */
			if ((IsClearGround(ti->tile, CLEAR_DESERT) || IsClearGround(ti->tile, CLEAR_SNOW)) && GetClearDensity(ti->tile) >= (ti->tileh & track_slope ? 1u : 2u)) {
				image += rti->snow_offset;
			} else if (GetClearDensity(ti->tile) == 0) {
		...
		if (_game_mode != GM_MENU && _settings_client.gui.show_track_reservation) {
			...
			TrackBits pbs = GetRailReservationTrackBits(ti->tptr) & track;
			if (pbs & TRACK_BIT_X) {
		...
		if (IsTileType(ti->tile, MP_CLEAR)) {
			/* Adjust drawn sprite for better appearance. */
			if ((IsClearGround(ti->tile, CLEAR_DESERT) || IsClearGround(ti->tile, CLEAR_SNOW)) && GetClearDensity(ti->tile) >= 1) {
				image += rti->snow_offset;
			} else if (GetClearDensity(ti->tile) == 0) {
				pal = PALETTE_TO_BARE_LAND;
			}
...
static void DrawSignals(TileIndex tile, const Tile *rail_tile, TrackBits rails, const RailtypeInfo *rti)
{
#define MAYBE_DRAW_SIGNAL(x, y, z, t) if (IsSignalPresent(rail_tile, x)) DrawSingleSignal(tile, rail_tile, rti, t, GetSingleSignalState(rail_tile, x), y, z)
...
static void DrawTile_Track(TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	if (IsPlainRail(ti->tptr)) {
		...
		if (!rti->UsesOverlay() && IsCrossingBarred(ti->tptr)) image += 2;
		...
		if (rti->UsesOverlay()) {
			...
			if (tram_rti != nullptr || road_rti->UsesOverlay()) {
				/* Add normal rail overlay, unless there is only the base road sprite. */
				DrawGroundSprite(axis == AXIS_X ? rti->base_sprites.single_x : rti->base_sprites.single_y, PAL_NONE);
			}
		...
		if (rti->UsesOverlay()) {
			...
			if (image != 0) {
				...
				if (IsTileType(ti->tile, MP_CLEAR) && IsClearGround(ti->tile, CLEAR_DESERT) && GetClearDensity(ti->tile) >= 2) image += rti->snow_offset;
			}
		...
		if (HasRailCatenaryDrawn(GetRailType(ti->tptr))) DrawRailCatenary(ti);
...
static Foundation GetFoundation_Track(TileIndex tile, Tile *rail_tile, Slope tileh)
{
...
static bool TileLoop_Track(TileIndex tile, Tile *&rail_tile)
{
	...
	if (IsPlainRail(rail_tile) && (!IsTileType(tile, MP_CLEAR) || GetClearDensity(tile) == 3)) { // wait until bottom is green
		...
		for (DiagDirection d = DIAGDIR_BEGIN; d < DIAGDIR_END; d++) {
			...
			if (ptr2 == nullptr) ptr2 = HasTileByType(tile2, MP_RAILWAY) ? GetTileByType(tile2, MP_RAILWAY) : _m.ToTile(tile2);

			/* Show fences if it's a house, industry, object, road, tunnelbridge or not owned by us. */
			if (!IsValidTile(tile2) || IsTileType(tile2, MP_HOUSE) || IsTileType(tile2, MP_INDUSTRY) ||
					IsTileType(tile2, MP_ROAD) || (IsTileType(tile2, MP_OBJECT) && !IsObjectType(tile2, OBJECT_OWNED_LAND)) || IsTileType(tile2, MP_TUNNELBRIDGE) || !IsTileOwner(ptr2, owner)) {
				fences |= 1 << d;
			}
		}
...
static TrackStatus GetTileTrackStatus_Track(TileIndex tile, Tile *rail_tile, TransportType mode, uint sub_mode, DiagDirection side)
{
	...
	switch (GetRailTileType(rail_tile)) {
		...
		case RAIL_TILE_SIGNALS: {
			trackbits = GetTrackBits(rail_tile);
			byte a = GetPresentSignals(rail_tile);
			uint b = GetSignalStates(rail_tile);
			...
			if (!IsOnewaySignal(rail_tile, TRACK_UPPER) || (a & SignalOnTrack(TRACK_UPPER)) == 0) b |= ~a & SignalOnTrack(TRACK_UPPER);
			if (!IsOnewaySignal(rail_tile, TRACK_LOWER) || (a & SignalOnTrack(TRACK_LOWER)) == 0) b |= ~a & SignalOnTrack(TRACK_LOWER);
...
static void GetTileDesc_Track(TileIndex tile, Tile *rail_tile, TileDesc *td)
{
	const RailtypeInfo *rti = GetRailTypeInfo(GetRailType(rail_tile));
	td->rail_speed = rti->max_speed;
	td->railtype = rti->strings.name;
	td->owner[0] = GetTileOwner(rail_tile);
	...
	switch (GetRailTileType(rail_tile)) {
		...
		case RAIL_TILE_SIGNALS: {
			...
			if (HasSignalOnTrack(rail_tile, TRACK_UPPER)) {
				primary_signal = GetSignalType(rail_tile, TRACK_UPPER);
				secondary_signal = HasSignalOnTrack(rail_tile, TRACK_LOWER) ? GetSignalType(rail_tile, TRACK_LOWER) : primary_signal;
			} else {
				secondary_signal = primary_signal = GetSignalType(rail_tile, TRACK_LOWER);
			}
...
{
	if (!IsTileOwner(rail_tile, old_owner)) return !MergeRailTiles(tile, rail_tile);
	...
	if (new_owner != INVALID_OWNER) {
		...
		if (IsLevelCrossing(rail_tile)) num_pieces *= LEVELCROSSING_TRACKBIT_FACTOR;
		RailType rt = GetRailType(rail_tile);
		Company::Get(old_owner)->infrastructure.rail[rt] -= num_pieces;
		Company::Get(new_owner)->infrastructure.rail[rt] += num_pieces;

		if (HasSignals(rail_tile)) {
			uint num_sigs = CountBits(GetPresentSignals(rail_tile));
			Company::Get(old_owner)->infrastructure.signal -= num_sigs;
			Company::Get(new_owner)->infrastructure.signal += num_sigs;
		}

		SetTileOwner(rail_tile, new_owner);

		return !MergeRailTiles(tile, rail_tile);
	} else {
		bool deleted;
...
static VehicleEnterTileStatus VehicleEnter_Track(Vehicle *u, TileIndex tile, Tile *rail_tile, int x, int y)
{
	...
	if (u->type != VEH_TRAIN || !IsRailDepot(rail_tile)) return VETSB_CONTINUE;
 ...
 *
 * @param ground_tile Tile index of the ground tile.
 * @param tile The tile.
 ...
 */
static CommandCost TestAutoslopeOnRailTile(TileIndex ground_tile, Tile *tile, uint flags, int z_old, Slope tileh_old, int z_new, Slope tileh_new, TrackBits rail_bits)
{
	...
	if (CheckRailSlope(tileh_new, rail_bits, TRACK_BIT_NONE, ground_tile).Failed()) return_cmd_error(STR_ERROR_MUST_REMOVE_RAILROAD_TRACK);
	...
	if (tileh_old != tileh_new) {
		...
		if (IsTileType(ground_tile, MP_WATER) && IsSlopeWithOneCornerRaised(tileh_old)) cost.AddCost(_price[PR_CLEAR_WATER]);
		if ((flags & DC_EXEC) != 0) MakeClearGrass(ground_tile);
	}
	return  cost;
}
...
static CommandCost TerraformTile_Track(TileIndex tile, Tile *rail_tile, DoCommandFlag flags, int z_new, Slope tileh_new)
{
	int z_old;
	...
	if (IsPlainRail(rail_tile)) {
		...
		bool was_water = (IsTileType(tile, MP_WATER) && IsSlopeWithOneCornerRaised(tileh_old));
		...
		CommandCost autoslope_result = TestAutoslopeOnRailTile(tile, rail_tile, flags, z_old, tileh_old, z_new, tileh_new, rail_bits);
		...
		} else if (AutoslopeCheckForEntranceEdge(tile, z_new, tileh_new, GetRailDepotDirection(rail_tile))) {
			return CommandCost(EXPENSES_CONSTRUCTION, _price[PR_BUILD_FOUNDATION]);
		}
	}
