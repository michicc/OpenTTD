...
{
	if (!IsPlainRail(_m.ToTile(tile))) return_cmd_error(STR_ERROR_IMPOSSIBLE_TRACK_COMBINATION);
...
CommandCost CmdBuildSingleRail(DoCommandFlag flags, TileIndex tile, RailType railtype, Track track, bool auto_remove_signals)
{
	...
	switch (GetTileType(tile)) {
		case MP_RAILWAY: {
			...
			if (HasSignals(rail_tile) && TracksOverlap(GetTrackBits(tile) | TrackToTrackBits(track))) {
				...
				for (Track track_it = TRACK_BEGIN; track_it < TRACK_END; track_it++) {
					if (HasTrack(tile, track_it) && HasSignalOnTrack(rail_tile, track_it)) {
...
CommandCost CmdBuildSingleSignal(DoCommandFlag flags, TileIndex tile, Track track, SignalType sigtype, SignalVariant sigvar, bool convert_signal, bool skip_existing_signals, bool ctrl_pressed, SignalType cycle_start, SignalType cycle_stop, uint8 num_dir_cycle, byte signals_copy)
{
	...
	if (flags & DC_EXEC) {
		...
		if (IsPbsSignal(sigtype)) {
			...
			SetSignalStates(rail_tile, (GetSignalStates(rail_tile) & ~mask) | ((HasBit(GetRailReservationTrackBits(tile), track) && EnsureNoVehicleOnGround(tile).Succeeded() ? UINT_MAX : 0) & mask));
		}
		MarkTileDirtyByTile(tile);
		AddTrackToSignalBuffer(tile, track, _current_company);
		YapfNotifyTrackLayoutChange(tile, track);
...
{
	Tile *rail_tile = GetTileByType(tile, MP_RAILWAY);
	if (!ValParamTrackOrientation(track) || !IsPlainRailTile(rail_tile) || !HasTrack(tile, track)) {
		return_cmd_error(STR_ERROR_THERE_IS_NO_RAILROAD_TRACK);
	}
	if (!HasSignalOnTrack(rail_tile, track)) {
...
{
	return IsPlainRail(rail_tile) ? GetRailFoundation(tileh, GetTrackBits(tile)) : FlatteningFoundation(tileh);
}
