...
void CheckOrders(const Vehicle *v)
{
	...
	if (v->owner == _local_company && v->day_counter % 20 == 0) {
		...
		AddConsistAdviceNewsItem(message, cs->index);
	}
}
...
{
	if (v->GetConsist() != NULL) DeleteOrderWarnings(v->GetConsist());
	...
	if (reset_order_indices) {
		if (v->GetConsist() != NULL) v->GetConsist()->cur_implicit_order_index = v->GetConsist()->cur_real_order_index = 0;
	...
	switch (order->GetType()) {
		...
		case OT_CONDITIONAL: {
			...
			if (next_order != INVALID_VEH_ORDER_ID) {
				...
				UpdateConsistTimetable(cs, false);
