 ...
 * @param *dst delete the orders of this consist
 * @param flags execution flags
 */
static CommandCost DecloneOrder(Consist *dst, DoCommandFlag flags)
{
	if (flags & DC_EXEC) {
		DeleteVehicleOrders(dst->Front());
		InvalidateConsistOrder(dst, VIWD_REMOVE_ALL_ORDERS);
...
CommandCost CmdDeleteOrder(DoCommandFlag flags, ConsistID consist, VehicleOrderID sel_ord)
{
	...
	if (sel_ord >= v->GetNumOrders()) return DecloneOrder(cs, flags);
	...
	switch (action) {
		case CO_SHARE: {
			...
			if (flags & DC_EXEC) {
				...
				InvalidateConsistOrder(dst, VIWD_REMOVE_ALL_ORDERS);
				InvalidateConsistOrder(src, VIWD_MODIFY_ORDERS);
