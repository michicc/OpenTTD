...
static void AddAcceptedCargo_Industry(TileIndex tile, Tile *ind_tile, CargoArray &acceptance, CargoTypes *always_accepted)
{
	IndustryGfx gfx = GetIndustryGfx(tile);
...
static bool ClickTile_Industry(TileIndex tile, Tile *tptr)
{
	ShowIndustryViewWindow(GetIndustryIndex(tile));
	return true;
}

static TrackStatus GetTileTrackStatus_Industry(TileIndex tile, Tile *tptr, TransportType mode, uint sub_mode, DiagDirection side)
{
	return 0;
}
...
static bool SearchLumberMillTrees(TileIndex tile, void *user_data)
{
	...
	if (tree_tile != nullptr && GetTreeGrowth(tree_tile) > 2) { ///< 3 and up means all fully grown trees
		/* found a tree */
...
static bool CheckCanTerraformSurroundingTiles(TileIndex tile, uint height, int internal)
{
	...
	for (TileIndex tile_walk : ta) {
		...
		/* Is the tile clear? */
		if (GetTileType(tile_walk) != MP_CLEAR || HasTileByType(tile_walk, MP_RAILWAY)) return false;
