...
static void DrawTile_Industry(TileInfo *ti, bool, Corner)
{
	...
	if (gfx >= NEW_INDUSTRYTILEOFFSET) {
		...
		if (indts->grf_prop.spritegroup[0] != nullptr && DrawNewIndustryTile(ti, ind, gfx)) {
			return;
		} else {
...
static std::tuple<CommandCost, bool> ClearTile_Industry(TileIndex, Tile &tile, DoCommandFlag flags)
{
	...
	if ((_current_company != OWNER_WATER && _game_mode != GM_EDITOR &&
		...
		return {CommandCost(flags & DC_AUTO ? STR_ERROR_GENERIC_OBJECT_IN_THE_WAY : INVALID_STRING_ID), false};
	}
...
static bool TileLoop_Industry(TileIndex index, Tile &tile)
{
	...
	TriggerIndustryTile(index, INDTILE_TRIGGER_TILE_LOOP);
	...
	if (TransportIndustryGoods(index) && !StartStopIndustryTileAnimation(Industry::GetByTile(tile), IAT_INDUSTRY_DISTRIBUTES_CARGO)) {
	...
	if (StartStopIndustryTileAnimation(index, IAT_TILELOOP)) return false;
...
static bool SearchLumberMillTrees(TileIndex tile, void *)
{
	...
	if (tree_tile && GetTreeGrowth(tree_tile) >= TreeGrowthStage::Grown) {
		/* found a tree */
