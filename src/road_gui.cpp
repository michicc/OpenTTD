...
void ConnectRoadToStructure(TileIndex tile, DiagDirection direction)
{
	...
	Tile *road_tile = GetRoadTileByType(tile, _cur_roadtype);
	if (IsNormalRoadTile(road_tile)) {
		if (GetRoadBits(road_tile) != ROAD_NONE) {
				...
				case DDSP_REMOVE_BUSSTOP:
					if (this->IsWidgetLowered(WID_ROT_BUS_STATION)) {
						if (_remove_button_clicked) {
							...
							PlaceRoadStop(start_tile, end_tile, (_ctrl_pressed << 5) | (_cur_roadtype << 2) | ROADSTOP_BUS, CMD_BUILD_ROAD_STOP | CMD_MSG(_road_type_infos[_cur_roadtype].err_build_station[ROADSTOP_BUS]));
						}
					}
					break;
				...
				case DDSP_REMOVE_TRUCKSTOP:
					if (this->IsWidgetLowered(WID_ROT_TRUCK_STATION)) {
						if (_remove_button_clicked) {
							...
							PlaceRoadStop(start_tile, end_tile, (_ctrl_pressed << 5) | (_cur_roadtype << 2) | ROADSTOP_TRUCK, CMD_BUILD_ROAD_STOP | CMD_MSG(_road_type_infos[_cur_roadtype].err_build_station[ROADSTOP_TRUCK]));
						}
					}
					break;
			}
