...
protected:
	Vehicle *front = nullptr; ///< Pointer to the first vehicle of the associated vehicle chain.

	Consist(ConsistID index, VehicleType type = VehicleType::Invalid, ::Owner owner = INVALID_OWNER) : ConsistPool::PoolItem<&_consist_pool>(index), type(type), owner(owner) {}
...
protected:
	...
	inline SpecializedConsist(ConsistID index, ::Owner owner = INVALID_OWNER) : Consist(index, TType, owner) {}
...
public:
	...
	{
		return Consist::Create<T>(std::forward<Targs &&>(args)...);
	}
	 ...
	 */
	template <typename... Targs>
	static inline T *CreateAtIndex(ConsistID index, Targs &&... args)
	{
		return Consist::CreateAtIndex<T>(index, std::forward<Targs &&>(args)...);
	}
