...
void DrawClearLandTile(const TileInfo *ti, byte set, bool draw_halftile, Corner halftile_corner)
{
	if (!draw_halftile) {
		...
		DrawGroundSprite(SPR_FLAT_BARE_LAND + SlopeToSpriteOffset(fake_slope) + set * 19, PAL_NONE, GetHalftileSubSprite(halftile_corner));
	}
}
...
void DrawHillyLandTile(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	if (!draw_halftile) {
		...
		/* Draw higher halftile-overlay: Use the sloped sprites with three corners raised. They probably best fit the lightning. */
		Slope fake_slope = SlopeWithThreeCornersRaised(OppositeCorner(halftile_corner));
		DrawGroundSprite(SPR_FLAT_ROUGH_LAND + SlopeToSpriteOffset(fake_slope), PAL_NONE, GetHalftileSubSprite(halftile_corner));
	}
}
...
static void DrawRockLandTile(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	if (!draw_halftile) {
		DrawGroundSprite(rocks + SlopeToSpriteOffset(ti->tileh), PAL_NONE);
	} else {
		/* Draw higher halftile-overlay: Use the sloped sprites with three corners raised. They probably best fit the lightning. */
		Slope fake_slope = SlopeWithThreeCornersRaised(OppositeCorner(halftile_corner));
		DrawGroundSprite(rocks + SlopeToSpriteOffset(fake_slope), PAL_NONE, GetHalftileSubSprite(halftile_corner));
	}
}
...
static void DrawSnowDesertTile(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	if (!draw_halftile) {
		...
		/* Draw higher halftile-overlay: Use the sloped sprites with three corners raised. They probably best fit the lightning. */
		Slope fake_slope = SlopeWithThreeCornersRaised(OppositeCorner(halftile_corner));
		/* If the tile has snow, increase the density by one to match the surrounding tiles. */
		uint density = GetClearDensity(ti->tile);
		if (IsSnowTile(ti->tile) && density < 3) density++;
		DrawGroundSprite(_clear_land_sprites_snow_desert[density] + SlopeToSpriteOffset(fake_slope), PAL_NONE, GetHalftileSubSprite(halftile_corner));
	}
}
...
static void DrawFieldTile(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	if (!draw_halftile) {
		...
		/* Draw higher halftile-overlay: Use the sloped sprites with three corners raised. They probably best fit the lightning. */
		Slope fake_slope = SlopeWithThreeCornersRaised(OppositeCorner(halftile_corner));
		DrawGroundSprite(_clear_land_sprites_farmland[GetFieldType(ti->tile)] + SlopeToSpriteOffset(fake_slope), PAL_NONE, GetHalftileSubSprite(halftile_corner));
	}
}
...
static void DrawClearLandFence(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	if (IsValidCorner(halftile_corner)) {
		/* Tile has a half-tile foundation. Draw a fence that touches the
		 * half-tile corner only when the half-tile is drawn and fences that
		 * don't touch only when the normal tile is drawn. */
...
static Foundation GetFoundation_Clear(TileIndex tile, Tile *tptr, Slope tileh)
{
...
{
	int h;
	GetFoundationSlope(tile, &h);
	int k = h - GetSnowLine() + 1;
...
static void GetTileDesc_Clear(TileIndex tile, Tile *tptr, TileDesc *td)
{
	if (IsClearGround(tile, CLEAR_GRASS) && GetClearDensity(tile) == 0) {
