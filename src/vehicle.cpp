...
void Vehicle::PreDestructor()
{
	...
	if (this->type == VEH_ROAD && this->IsPrimaryVehicle()) {
		...
		if (!(v->vehstatus & VS_CRASHED) && IsInsideMM(v->state, RVSB_IN_DT_ROAD_STOP, RVSB_IN_DT_ROAD_STOP_END)) {
			...
			RoadStop::GetByTile(v->tile, GetRoadStopType(GetTileByType(v->tile, MP_STATION)))->Leave(v);
		}
	}
...
VehicleEnterTileStatus VehicleEnterTile(Vehicle *v, TileIndex tile, int x, int y)
{
	...
	do {
		if (_tile_type_procs[GetTileType(tptr)]->vehicle_enter_tile_proc != NULL) {
			vets |= _tile_type_procs[GetTileType(tptr)]->vehicle_enter_tile_proc(v, tile, tptr, x, y);
		}
	} while (vets == VETSB_CONTINUE && HasAssociatedTile(tptr++));
	return vets;
}
 ...
 * @pre HasTileByType(this->tile, MP_STATION) || this->type == VEH_SHIP.
 */
void Vehicle::BeginLoading()
{
	assert(HasTileByType(this->tile, MP_STATION) || this->type == VEH_SHIP);
...
void Vehicle::LeaveStation()
{
	...
	if (this->type == VEH_TRAIN && !(this->vehstatus & VS_CRASHED)) {
		...
		if (HasTileByType(this->tile, MP_STATION)) {
