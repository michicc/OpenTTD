...
protected:
	...
	struct TILE {
		...
		TILE(TileIndex tile, Trackdir td, Tile *rail = NULL)
		{
			this->tile = tile;
			this->td = td;
...
public:
	...
	inline int SwitchCost(TileIndex tile1, TileIndex tile2, DiagDirection exitdir)
	{
		...
		if (IsNormalRailTile(p1) && IsNormalRailTile(p2)) {
	...
	inline int OneTileCost(const Tile *tile, Trackdir trackdir)
	{
		/* set base cost */
			...
			if (segment_cost == 0) {
				...
				if (is_cached_segment) {
					...
					if (segment.m_last_signal_tile != INVALID_TILE) {
						...
						assert(rail_tile != NULL && HasSignalOnTrackdir(rail_tile, segment.m_last_signal_td));
...
no_entry_cost: // jump here at the beginning if the node has no parent (it is the first node)
			...
			} else if (TrackFollower::DoTrackMasking() && cur.rail_tile != NULL) {
