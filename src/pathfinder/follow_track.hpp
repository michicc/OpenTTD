...
struct CFollowTrackT
{
	...
	Tile               *m_new_tile_ptr;  ///< pointer to the actual new tile
...
protected:
	...
	inline bool QueryNewTileTrackStatus()
	{
		if (IsRailTT() && HasTileByType(m_new_tile, MP_RAILWAY)) {
			m_new_tile_ptr = GetTileByType(m_new_tile, MP_RAILWAY);
			/* We have at least one rail tile. */
			do {
				m_new_td_bits = GetRailTrackdirBits(m_new_tile_ptr, m_exitdir);
			} while (m_new_td_bits == TRACKDIR_BIT_NONE && (m_new_tile_ptr = GetNextTileByType(m_new_tile_ptr, MP_RAILWAY)) != nullptr);
		} else if (IsRoadTT()) {
	...
	inline bool CanEnterNewTile()
	{
		...
		if (IsRailTT()) {
			Tile *rail_tile = m_new_tile_ptr;
			if (!IsRailTile(rail_tile)) rail_tile = _m.ToTile(m_new_tile);
