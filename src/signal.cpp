...
public:
	 ...
	 */
	bool Add(TileIndex tile, Tdir dir, Tile *tptr = NULL)
	{
		if (this->IsFull()) {
			overflowed = true;
		...
		this->data[this->n].tptr = tptr == NULL ? _m.ToTile(tile) : tptr;
		this->n++;
	 ...
	 */
	bool Get(TileIndex *tile, Tdir *dir, Tile **tptr = NULL)
	{
		...
		*dir = this->data[this->n].dir;
		if (tptr != NULL) *tptr = this->data[this->n].tptr;
...
static SigSegState UpdateSignalsInBuffer(Owner owner)
{
	...
	while (_globset.Get(&tile, &dir)) {
		...
		if (tptr == NULL) tptr = _m.ToTile(tile);
