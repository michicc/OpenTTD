...
static TrackBits GetSingleRailTrackBitsUniversal(TileIndex t, Tile *rail_tile, byte *override)
{
	if (IsTileType(rail_tile, MP_RAILWAY)) {
		if (!IsNormalRail(rail_tile) || !HasCatenary(GetRailType(rail_tile))) return TRACK_BIT_NONE;
...
static TrackBits GetRailTrackBitsUniversal(TileIndex t, byte *override)
{
	if (HasTileByType(t, MP_RAILWAY)) {
		...
		FOR_ALL_RAIL_TILES(rail_tile, t) {
			if (IsNormalRail(rail_tile) && HasCatenary(GetRailType(rail_tile))) tracks |= GetTrackBits(rail_tile);
		}
		return tracks;
	}
 ...
 */
static inline SpriteID GetWireBase(TileIndex tile, Tile *tptr, TileContext context = TCX_NORMAL)
{
 ...
 */
static inline SpriteID GetPylonBase(TileIndex tile, Tile *tptr, TileContext context = TCX_NORMAL)
{
	const RailtypeInfo *rti = GetRailTypeInfo(GetRailType(tptr));
 ...
 */
static void DrawCatenaryRailway(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	for (DiagDirection i = DIAGDIR_BEGIN; i < DIAGDIR_END; i++) {
		...
		tileh[TS_NEIGHBOUR] = GetFoundationSlope(neighbour);
 ...
 */
void DrawCatenary(const TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	switch (GetTileType(ti->tptr)) {
		case MP_RAILWAY:
			if (IsRailDepot(ti->tptr)) {
				const SortableSpriteStruct *sss = &CatenarySpriteData_Depot[GetRailDepotDirection(ti->tptr)];
	...
	DrawCatenaryRailway(ti, draw_halftile, halftile_corner);
}
