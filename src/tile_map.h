...
{
	return MayHaveAssociatedTile(tile) && HasBit(tile->m6, 0);
}
 ...
 * @pre MayHaveAssociatedTile(tile)
 */
 ...
 * @return The associated tile having the asked tile type or nullptr if no such tile exists.
 */
 ...
 * @param type Tile type to search for.
 * @return The next tile with the asked tile type or nullptr if no such tile exists.
 */
...
{
	return GetTileByType(tile, type) != nullptr;
}
