...
void ResizeWindow(Window *w, int delta_x, int delta_y, bool clamp_to_screen)
{
	if (delta_x != 0 || delta_y != 0) {
		if (clamp_to_screen) {
			...
			if (new_right  >= (int)_screen.width)  delta_x -= Ceil(new_right  - _screen.width,  max(1U, w->nested_root->resize_x));
			if (new_bottom >= (int)_screen.height) delta_y -= Ceil(new_bottom - _screen.height, max(1U, w->nested_root->resize_y));
		}
