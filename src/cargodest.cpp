...
{
	switch (type) {
		case SourceType::Town: return Town::Get(id);
		case SourceType::Industry: return Industry::Get(id);
		default: NOT_REACHED();
	}
}
...
const CargoLink *GetRandomCargoLink(const CargoSourceSink *source, CargoID cid, bool allow_self, bool allow_random, SourceType dst_type)
{
	...
	for (const auto &l : source->cargo_links[cid]) {
		...
		if (l.dest != nullptr && dst_type != SourceType::Undefined && l.dest->GetType() != dst_type) continue;
		if (l.dest != nullptr && !l.dest->IsCargoAccepted(cid)) continue;
