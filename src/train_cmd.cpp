...
void UpdateLevelCrossing(TileIndex tile, bool sound)
{
	...
	bool new_state = HasVehicleOnPos(tile, NULL, &TrainOnTileEnum) || TrainApproachingCrossing(tile) || GetRailReservationTrackBits(rail_tile) != TRACK_BIT_NONE;
	...
	if (p2 != 0) {
		...
		if (flags & DC_EXEC) {
			...
			if (v->current_order.IsType(OT_LOADING)) {
				...
				if (!HasTileByType(last->tile, MP_STATION) || GetStationIndex(last->tile) != GetStationIndex(v->tile)) {
					v->LeaveStation();
				}
			}
...
static bool CheckTrainStayInDepot(Train *v)
{
	...
	if (v->current_order.IsType(OT_GOTO_DEPOT) && v->tile == v->dest_tile) {
		...
		if (HasDepotReservation(depot_tile)) return true;
...
bool TryPathReserve(Train *v, bool mark_as_stuck, bool first_tile_okay)
{
	...
	if (v->track == TRACK_BIT_DEPOT) {
		Tile *depot_tile = GetRailDepotTile(v->tile);
		if (HasDepotReservation(depot_tile)) {
			if (mark_as_stuck) MarkTrainAsStuck(v);
			return false;
		} else {
			/* Depot not reserved, but the next tile might be. */
			TileIndex next_tile = TileAddByDiagDir(v->tile, GetRailDepotDirection(depot_tile));
			if (HasReservedTracks(next_tile, DiagdirReachesTracks(GetRailDepotDirection(depot_tile)))) return false;
		}
	}
...
{
	Tile *rail = GetRailTileFromDiagDir(tile, dir);
	if (rail != NULL &&	HasSignals(rail)) {
		...
		if (v->track != TRACK_BIT_WORMHOLE) {
			...
			if (gp.old_tile == gp.new_tile) {
				...
				if (prev == NULL) {
					...
					Track track = ChooseTrainTrack(v, gp.new_tile, enterdir, bits, false, NULL, true);
				...
				if (!HasBit(r, VETS_ENTERED_WORMHOLE)) {
					...
					if (v->IsFrontEngine() && HasPbsSignalOnTrackdir(gp.new_tile, tdir)) {
						SetSignalStateByTrackdir(GetRailTileFromTrack(gp.new_tile, track), tdir, SIGNAL_STATE_RED);
	...
	if (j < adv_spd) {
		...
		for (;;) {
			...
			if ((order_type == OT_GOTO_WAYPOINT || order_type == OT_GOTO_STATION) &&
						...
						HasTileByType(v->tile, MP_STATION) &&
