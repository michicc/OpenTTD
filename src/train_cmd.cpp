...
static CommandCost CmdBuildRailWagon(DoCommandFlag flags, TileIndex tile, const Engine *e, Vehicle **ret)
{
	...
	if (!IsRailDepotTile(tile) || !IsCompatibleRail(rvi->railtype, GetRailType(GetRailDepotTile(tile)))) return CMD_ERROR;
...
void UpdateLevelCrossing(TileIndex tile, bool sound)
{
	...
	bool new_state = GetRailReservationTrackBits(rail_tile) != TRACK_BIT_NONE || HasVehicleOnPos(tile, nullptr, &TrainOnTileEnum) || TrainApproachingCrossing(tile);
...
static TileIndex TrainApproachingCrossingTile(const Train *v)
{
	...
	if (!IsLevelCrossingTile(tile) || GetRailTileFromDiagDir(tile, dir) == nullptr ||
