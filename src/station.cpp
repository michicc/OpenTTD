...
Station::~Station()
{
	...
	for (CargoID c = 0; c < NUM_CARGO; ++c) {
		...
		for (NodeID node = 0; node < lg->Size(); ++node) {
			...
			auto edges = (*lg)[node].GetEdgesTo(this->goods[c].node);
			if (std::any_of(edges.first, edges.second, [] (const LinkGraph::BaseEdge &e) { return e.LastUpdate() != CalendarTime::INVALID_DATE; })) {
