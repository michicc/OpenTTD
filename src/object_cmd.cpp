	...
	switch (type) {
		...
		case OBJECT_HQ: {
			...
			if (c->location_of_HQ != INVALID_TILE) {
				...
				cost.AddCost(ClearTile_Object(c->location_of_HQ, _m.ToTile(c->location_of_HQ), flags, NULL));
...
static void AddAcceptedCargo_Object(TileIndex tile, Tile *tptr, CargoArray &acceptance, uint32 *always_accepted)
{
...
static bool ClickTile_Object(TileIndex tile, Tile *tptr)
{
...
static void AnimateTile_Object(TileIndex tile, Tile *tptr)
{
