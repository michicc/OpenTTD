...
static std::tuple<CommandCost, bool> ClearTile_Object(TileIndex index, Tile &tile, DoCommandFlag flags);
...
static void DrawTile_Object(TileInfo *ti)
{
	...
	if ((spec->flags & OBJECT_FLAG_HAS_NO_FOUNDATION) == 0) DrawFoundation(ti, GetFoundation_Object(ti->index, ti->tileh));
...
static std::tuple<CommandCost, bool> ClearTile_Object(TileIndex index, Tile &tile, DoCommandFlag flags)
{
	...
	if (_current_company != OWNER_WATER) {
		if ((flags & DC_NO_WATER) && IsTileOnWater(tile)) {
			...
			if (!_cheats.magic_bulldozer.value && (spec->flags & OBJECT_FLAG_CANNOT_REMOVE) != 0) return {CMD_ERROR, false};
	...
	_cleared_object_areas.push_back({index, ta});
...
static bool TileLoop_Object(TileIndex index, Tile &tile)
{
	...
	if (spec->flags & OBJECT_FLAG_ANIMATION) {
		Object *o = Object::GetByTile(tile);
		TriggerObjectTileAnimation(o, index, OAT_TILELOOP, spec);
		if (o->location.tile == index) TriggerObjectAnimation(o, OAT_256_TICKS, spec);
	}
...
static CommandCost TerraformTile_Object(TileIndex index, Tile tile, DoCommandFlag, int z_new, Slope tileh_new)
{
	ObjectType type = GetObjectType(tile);
