...
bool CargoLinkSorter(const CargoSourceSink *cur_css, const GUICargoLink &a, const GUICargoLink &b)
{
	...
	auto name = GetString(a.link->dest->GetType() == SourceType::Town ? STR_TOWN_NAME : STR_INDUSTRY_NAME);
	...
	if (b.link != last_b) {
		...
		last_name = GetString(b.link->dest->GetType() == SourceType::Town ? STR_TOWN_NAME : STR_INDUSTRY_NAME);
	}
...
{
	size_t lines = 1 + this->link_list.size();
...
StringID CargoDestinationList::PrepareDisplayString(const GUICargoLink &l) const
{
	...
	return l.link->dest->GetType() == SourceType::Town ? STR_VIEW_CARGO_LAST_MONTH_TOWN : STR_VIEW_CARGO_LAST_MONTH_INDUSTRY;
}
