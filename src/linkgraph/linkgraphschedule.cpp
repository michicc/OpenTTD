...
void StateGameLoop_LinkGraphPauseControl()
{
	if (_pause_mode & PM_PAUSED_LINK_GRAPH) {
		...
		if (!LinkGraphSchedule::instance.IsJoinWithUnfinishedJobDue()) {
			...
			_date % (_settings_game.cargo.linkgraph.recalc_interval / SECONDS_PER_DAY) == (_settings_game.cargo.linkgraph.recalc_interval / SECONDS_PER_DAY) / 2 &&
...
void OnTick_LinkGraph()
{
	...
	Date offset = _date % (_settings_game.cargo.linkgraph.recalc_interval / SECONDS_PER_DAY);
	if (offset == 0) {
		LinkGraphSchedule::instance.SpawnNext();
	} else if (offset == (_settings_game.cargo.linkgraph.recalc_interval / SECONDS_PER_DAY) / 2) {
