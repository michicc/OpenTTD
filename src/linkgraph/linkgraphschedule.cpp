...
void StateGameLoop_LinkGraphPauseControl()
{
	if (_pause_mode & PM_PAUSED_LINK_GRAPH) {
		...
		if (!LinkGraphSchedule::instance.IsJoinWithUnfinishedJobDue()) {
			...
			TimerGameCalendar::date.base() % (_settings_game.cargo.linkgraph.recalc_interval / CalendarTime::SECONDS_PER_DAY) == (_settings_game.cargo.linkgraph.recalc_interval / CalendarTime::SECONDS_PER_DAY) / 2 &&
...
void OnTick_LinkGraph()
{
	...
	TimerGameCalendar::Date offset = TimerGameCalendar::date.base() % (_settings_game.cargo.linkgraph.recalc_interval / CalendarTime::SECONDS_PER_DAY);
	if (offset == 0) {
		LinkGraphSchedule::instance.SpawnNext();
	} else if (offset == (_settings_game.cargo.linkgraph.recalc_interval / CalendarTime::SECONDS_PER_DAY) / 2) {
