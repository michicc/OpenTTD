...
LinkGraphJob::LinkGraphJob(const LinkGraph &orig) :
		...
		join_date(TimerGameCalendar::date + (_settings_game.cargo.linkgraph.recalc_time / CalendarTime::SECONDS_PER_DAY)),
		job_completed(false),
		job_aborted(false)
{
}
	...
	for (NodeID node_id = 0; node_id < size; ++node_id) {
		...
		for (const auto &edge : from.edges) {
			...
			if (st2 == nullptr || st2->goods[this->Cargo()].link_graph != this->link_graph.index ||
					...
					(*lg)[node_id].GetEdge(dest_id, edge.base.from_order, edge.base.dest_order).LastUpdate() == CalendarTime::INVALID_DATE) {
			...
			} else if ((*lg)[node_id].GetEdge(dest_id, edge.base.from_order, edge.base.dest_order).last_restricted_update == CalendarTime::INVALID_DATE) {
		...
		for (FlowStatMap::iterator it(ge.flows.begin()); it != ge.flows.end();) {
			...
			if (new_it == flows.end()) {
				if (_settings_game.cargo.GetDistributionType(this->Cargo()) != DT_MANUAL && _settings_game.cargo.GetDistributionType(this->Cargo()) != DT_FIXED) {
