 ...
 */
void IncreaseStats(Station *st, CargoID cargo, StationID next_station_id, OrderID from_oid, OrderID to_oid, uint capacity, uint usage, uint32_t time, EdgeUpdateMode mode)
{
	...
	if (lg != nullptr) {
		/* Track orders for edges only when using fixed cargo distribution. */
		bool fixed_dest = _settings_game.cargo.GetDistributionType(cargo) == DT_FIXED;
		(*lg)[ge1.node].UpdateEdge(ge2.node, fixed_dest ? from_oid : INVALID_ORDER, fixed_dest ? to_oid : INVALID_ORDER, capacity, usage, time, mode);
	}
}
 ...
 * @param to_oid Destination order.
 */
void IncreaseStats(Station *st, const Vehicle *front, StationID next_station_id, OrderID from_oid, OrderID to_oid, uint32_t time)
{
