		...
		TILE_AREA_LOOP(cur_tile, roadstop_area) {
			...
			*currstop = road_stop;
			...
			if (is_drive_through) {
				...
				/* Clear all existing associated tiles and create the new station tile. */
				DoClearSquare(tile);
...
CommandCost CmdRemoveRoadStop(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	TILE_AREA_LOOP(cur_tile, roadstop_area) {
		...
		RoadBits road_bits = IsDriveThroughStop(st_tile) ?
				((GetRoadStopDir(st_tile) == DIAGDIR_NE) ? ROAD_X : ROAD_Y) :
				DiagDirToRoadBits(GetRoadStopDir(st_tile));
		...
		if ((flags & DC_EXEC) && is_drive_through) {
			...
			TownID town = ClosestTownFromTile(cur_tile, UINT_MAX)->index;
			if (HasBit(rts, ROADTYPE_ROAD)) MakeRoadNormal(tile, road_bits, ROADTYPE_ROAD, town, road_owner);
			if (HasBit(rts, ROADTYPE_TRAM)) MakeRoadNormal(tile, road_bits, ROADTYPE_TRAM, town, tram_owner);
		}
	}
...
static void DrawTile_Station(TileInfo *ti, bool draw_halftile, Corner halftile_corner)
{
	...
	if (HasStationRail(ti->tptr) && HasCatenaryDrawn(GetRailType(ti->tile))) DrawCatenary(ti);
